...
class BlastAnti1kResult(BlastAntismash1k):
    ...
    def cunei_B12(self, gene_clu_tong_blastdf, df_blaster_filter, merge_df_filter):
        gene_clu_tong_blastdf.drop("同源蛋白", axis=1, inplace=True)
        merge_df_filter['query acc.ver'] = merge_df_filter['同源蛋白']
        cunei_b1 = pd.merge(merge_df_filter.copy(), gene_clu_tong_blastdf.copy(),
                            on="query acc.ver", suffixes=("_b1", "_b2"))
        cunei_b1.drop("query acc.ver", axis=1, inplace=True)
        cunei_b1.rename(columns={"同源蛋白": "query"}, inplace=True)
        pro_b = cunei_b1['query'].tolist()
        blast_filter_b1 = df_blaster_filter[df_blaster_filter['subject acc.ver'].isin(pro_b)]
        print(blast_filter_b1)
        blast_filter_b1_copy = blast_filter_b1.copy()
        blast_filter_b1_copy['query'] = blast_filter_b1_copy['subject acc.ver']
        merge_cune = pd.merge(blast_filter_b1_copy, cunei_b1.copy(), on="query", suffixes=("_a", "_b"))
        merge_cune.to_excel("ne.xlsx")
    ...
    def main(self, datapath):
        for bl in os.listdir(self.tmp_blast):
            ...
            print(genecluster_tongyuna_blast_df)
            self.cunei_B12(genecluster_tongyuna_blast_df, df_blast_filter.copy(), merge_df_filter.copy())
            ...
            all_bc.to_excel( "all.xlsx", index=False)
            break
